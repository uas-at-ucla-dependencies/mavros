...
public:
	...
	void initialize(UAS &uas_)
	{
		...
		// general params
		frame_id = gp_nh->declare_parameter<std::string>("global_position/frame_id", "map");
		child_frame_id = gp_nh->declare_parameter<std::string>("global_position/child_frame_id", "base_link");
		rot_cov = gp_nh->declare_parameter("global_position/rot_covariance", 99999.0);
		gps_uere = gp_nh->declare_parameter("global_position/gps_uere", 1.0);
		use_relative_alt = gp_nh->declare_parameter("global_position/use_relative_alt", true);
		// tf subsection
		tf_send = gp_nh->declare_parameter("global_position/tf/send", false);
		tf_frame_id = gp_nh->declare_parameter<std::string>("global_position/tf/frame_id", "map");
		tf_global_frame_id = gp_nh->declare_parameter<std::string>("global_position/tf/global_frame_id", "earth");	// The global_origin should be represented as "earth" coordinate frame (ECEF) (REP 105)
		tf_child_frame_id = gp_nh->declare_parameter<std::string>("global_position/tf/child_frame_id", "base_link");
		...
		gp_set_global_origin_sub = 
			gp_nh->create_subscription<geographic_msgs::msg::GeoPointStamped>("set_gp_origin", 10, 
				std::bind(&GlobalPositionPlugin::set_gp_origin_cb, this, std::placeholders::_1));

		// home position subscriber to set "map" origin
		// TODO use UAS
		hp_sub = gp_nh->create_subscription<mavros_msgs::msg::HomePosition>("home", 10,
			std::bind(&GlobalPositionPlugin::home_position_cb, this, std::placeholders::_1));
